...
function Pieces() {
  ...
  useEffect(() => {
    if (
      isAnimating &&
      (Object.keys(animatingPieces).length > 0 ||
        Object.keys(fadingPieces).length > 0 ||
        Object.keys(promotingPieces).length > 0)
    ) {
      const animateFrame = () => {
  ...
  function getSquarePosition(squareIndex: number) {
    ...
    const row = roleRef.current
      ? 7 - Math.floor(squareIndex / 8)
      : Math.floor(squareIndex / 8);
    return { col, row };
  }
      ...
      <div className="relative w-full h-full">
        {boardRef.current.map((piece, i) => {
          if (piece !== Piece.NO_PIECE) {
            ...
            if (isAnimatingFromHere || isFadingHere || isPromotingHere)
              return null;
